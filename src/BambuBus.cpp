...
uint8_t online_detect_num5[] = {0xBF, 0xCF, 0x30, 0x35, 0x39, 0x37, 0x18, 0x00, 0x4A, 0x36, 0x30, 0x35, 0xFF, 0xFF, 0xFF, 0xFF};
...
void BambuBus_init()
{
    ...
    online_detect_num1[0] += (hub_num - 1);
    online_detect_num2[0] += (hub_num - 1);
    online_detect_num3[0] += (hub_num - 1);
    online_detect_num4[0] += (hub_num - 1);
...
void set_motion_res_datas(unsigned char *set_buf, unsigned char AMS_num, unsigned char read_num, unsigned char statu_flags)
{
    ...
    if ((read_num != 0xFF) && (read_num < 4))
    {
        ...
        else if ((data_save.filament[AMS_num][read_num].motion_set == need_send_out) || (data_save.filament[AMS_num][read_num].motion_set == need_pull_back)) // sending or pull back
        {
            motion_flag = 0x02;
            pressure = 0x4700;
            if (last_meters == 0)
                last_meters = filament_meters[AMS_num][read_num];
            meters += (filament_meters[AMS_num][read_num] - last_meters);
        }
...
bool set_motion(unsigned char AMS_num, unsigned char read_num, unsigned char statu_flags, unsigned char fliment_motion_flag)
{
    ...
    if (AMS_num == data_save.BambuBus_now_filament_num / 4)
        time_last = time_now;
    if (BambuBus_address == BambuBus_AMS) // AMS08
    {
        ...
        else if ((read_num == 0xFF))
        {
            ...
            else if ((statu_flags == 0x01) && (fliment_motion_flag == 0x00)) // 01 00(FF)
            {
                for (auto i = 0; i < 4; i++)
                {
                    if (data_save.filament[AMS_num][i].motion_set != on_use || idle_count > 9999)
...
package_type BambuBus_run()
{
    ...
    static uint32_t time_set = 0;
    static uint32_t time_motion = 0;
    static uint32_t time_long_motion = 0;
    update_time();
    uint32_t timex = get_time32();
    Bmcu_run();
