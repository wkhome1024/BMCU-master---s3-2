...
bool IRAM_ATTR capture_isr(mcpwm_unit_t unit, mcpwm_capture_channel_id_t cap_channel, const cap_event_data_t *data, void* user){
    ...
    : MCPWM_unit(_MCPWM_unit) , input_pwm_pin(_input_pwm_pin) , capture_channel(_capture_channel)
{
    if((MCPWM_unit < 2 && MCPWM_unit > -1) && (capture_channel < 3 && capture_channel > -1)){
        int channel_index = MCPWM_unit * 3 + capture_channel;
        
        if(captures >> channel_index & CAP_MASK)
            log_w("Selected MCPWM_UNIT and MCPWM_CAP have previous config. Resetting config.");
            
        reset_captures(channel_index);
        ...
        set_captures(channel_index);
    } else {
        log_e("Invalid MCPWM unit or capture channel choice. Initialization ABORTED.");
    }
}
...
double PWM_Analyzer::Get_PWM_duty_cycle(){ 
    if(high == 0 && low == 0){
        ...
        duty_cycle = (double)high * 100.0 / (high + low);
        return duty_cycle;
    }
}
